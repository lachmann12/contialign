...
// cargo build --release
// "target/release/tempest.exe" --index -f "files/Homo_sapiens.GRCh38.cdna.all.fa" -k 22 -o "output/myindex_24.idx"
// "target/release/tempest.exe" -f "files/SRR3534129.fastq" -o "output/counts3.tsv" -x "output/myindex_24.idx"

// "target/release/tempest" --index -f "files/Homo_sapiens.GRCh38.cdna.all.fa" -k 22 -o "output/myindex_24.idx"
// "target/release/tempest" -f "files/SRR3534129.fastq" -o "output/counts3.tsv" -x "output/myindex_24.idx"
...
fn main() -> Result<(), Error> {
    ...
    if build_index{
        ...
        let (transcripts, eq_classes, eq_elements, transcript_kmers, transcript_length) = parsefa::read_fa(input_file, kmer_length as usize);
        ...
        let (line_count, alignment_matches) = align::read_fastq(fastq_file, kmer_length as usize, &eq_classes, &eq_elements, &kmer_offset, step_size, sensitivity);
        ...
        println!("{}", format!("[{}] Done! Elapsed time: {}m {}s", Local::now().format("%Y-%m-%d][%H:%M:%S"), now.elapsed().as_millis()/60000, (now.elapsed().as_millis()%60000)/1000).green());
        println!("{}", format!("Transcripts: {} | Reads: {} | Aligned reads: {} | Success: {}%", transcripts.len(), line_count/4, total_count as usize, (100.0*total_count/(line_count/4) as f32) as usize).green());
    }
    Ok(())
}
